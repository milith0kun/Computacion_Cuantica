...
\subsection{Autenticaci\'on y Uso de la API IBM Quantum}
Para la interacci\'on program\'atica con el procesador f\'isico, se estableci\'o una conexi\'on a trav\'es de la API RESTful de IBM Quantum utilizando la clase \texttt{QiskitRuntimeService}. La clave de autenticaci\'on (\textit{API token}) se aisl\'o e inyect\'o en tiempo de ejecuci\'on mediante un gestor de credenciales local (\texttt{apikey.json}). Este flujo permiti\'o empaquetar y remitir los trabajos cu\'anticos (\textit{jobs}) optimizados localmente hacia la cola de la QPU, para as\'i descargar los resultados de la m\'aquina as\'incronamente.
